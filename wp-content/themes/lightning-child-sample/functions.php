...
if ($my_lightning_additional_css) {
	...
	add_action(
		...
		function () {
			...
			wp_enqueue_script('script', get_theme_file_uri('/assets/js/script.js'), [], false, true);
			wp_enqueue_style('bootstrap', get_theme_file_uri('/assets/bootstrap/css/bootstrap.min.css'));
			wp_enqueue_script('bootstrap', get_theme_file_uri('/assets/bootstrap/js/bootstrap.min.js'), [], false, true);
		}
	);
	// 編集画面側のCSSの読み込み.
	add_action(
